...
export const downloadAdviceJson = (advice: ResumeAdviceType, fileName = 'resume-advice.json'): void => {
  downloadFile(JSON.stringify(advice, null, 2), fileName, 'application/json');
};

export const downloadAdviceMarkdown = (advice: ResumeAdviceType, fileName = 'resume-advice.md'): void => {
  downloadFile(formatAdviceMarkdown(advice), fileName, 'text/markdown');
};

export const downloadAdviceReportHtml = (advice: ResumeAdviceType, fileName = 'resume-advice-report.html'): void => {
  downloadFile(formatAdviceReportHtml(advice), fileName, 'text/html');
};

export const downloadCoverLetterText = (text: string, fileName = 'cover-letter.txt'): void => {
  downloadFile(text, fileName, 'text/plain');
};

export const downloadResumeDoc = (text: string, fileName = 'resume.doc'): void => {
  downloadFile(formatResumeDocument('Resume', text), fileName, 'application/msword');
};

export const downloadResumePrintHtml = (text: string, fileName = 'resume-print.html'): void => {
  downloadFile(formatResumeDocument('Resume', text), fileName, 'text/html');
};

export const downloadTranslationMarkdown = (
...
export const downloadTranslationText = (
  translation: ResumeTranslationType,
  fileName = 'resume-translation.txt',
): void => {
